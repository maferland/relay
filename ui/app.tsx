      ...
      <div className="modal" role="dialog" aria-modal="true">
        ...
        <div className="modal-body">
          <div className="trans-preview">
            ...
            <StateBadge state={transition.to} size="sm" solid={transition.to === "blocked"} />
          </div>
...
function ToastHost({ toasts, onOpen, onDismiss }: { toasts: Toast[]; onOpen: (id: string) => void; onDismiss: (id: string) => void }) {
  return (
    <div className="toasts">
      {toasts.map((t) => (
        <div key={t.id} className={`toast ${t.kind}`}>
          ...
          <span className="iconbtn" style={{ width: 22, height: 22 }} onClick={() => onDismiss(t.id)}>
            <Icon name="x" size={13} />
          </span>
        </div>
      ))}
...
export function App() {
  const [dark, setDark] = useState(() => localStorage.getItem("at-theme") !== "light");
  const [snap, setSnap] = useState<Snapshot>(EMPTY);
  const [view, setView] = useState<View>("inbox");
  const [openId, setOpenId] = useState<string | null>(null);
  const [modal, setModal] = useState<{ task: UiTask; transition: Transition } | null>(null);
  ...
  function patchTask(updated: UiTask) {
    setSnap((s) => ({ ...s, tasks: s.tasks.map((t) => (t.id === updated.id ? updated : t)) }));
  }
  ...
  async function applyTransition(task: UiTask, transition: Transition, note: string) {
    setModal(null);
    try {
      ...
      pushToast({
        kind: transition.to === "done" ? "success" : transition.to === "blocked" ? "block" : "update",
        icon: transition.to === "done" ? "check" : transition.to === "blocked" ? "alert" : "arrowRight",
        title: "You " + verb,
        desc: task.title,
        taskId: task.id,
      });
    } catch (e) {
      pushToast({ kind: "block", icon: "alert", title: "Couldn't update", desc: e instanceof Error ? e.message : String(e) });
      refresh().catch(() => {});
    }
  }
  ...
  async function onResolve(task: UiTask) {
    try {
      ...
      pushToast({ kind: "success", icon: "check", title: "Resolved", desc: task.title, taskId: task.id });
    } catch (e) {
      pushToast({ kind: "block", icon: "alert", title: "Couldn't resolve", desc: e instanceof Error ? e.message : String(e) });
    }
  }
  ...
  const urgentCount = tasks.filter((t) => t.needsHuman || t.state === "blocked").length;
  ...
  return (
    <div className="app">
      <aside className="side">
        ...
        <nav className="nav">
          <button className={`nav-item ${view === "inbox" ? "is-active" : ""}`} onClick={() => goView("inbox")}>
            <Icon name="inbox" size={17} /> Needs you
            {needsYouCount > 0 && <span className={`nav-count ${urgentCount ? "urgent" : ""}`}>{needsYouCount}</span>}
          </button>
          <button
        ...
        <div className="side-projects">
          {projects.map((p) => {
            ...
            const b = tasks.filter((x) => x.project === p && (x.state === "blocked" || x.needsHuman)).length;
            return (
              <button key={p} className="nav-item" style={{ padding: "6px 10px" }} onClick={() => goView("board")}>
                <Icon name="folder" size={15} style={{ color: "var(--text-faint)" }} />
                <span className="mono" style={{ fontSize: "var(--fs-xs)" }}>
                  {p}
                </span>
                {b > 0 ? <span className="nav-count urgent">{b}</span> : <span className="nav-count">{c}</span>}
              </button>
            );
          })}
        </div>
        <div className="side-foot">
          <div className="meta" style={{ gap: 8 }}>
            ...
            <div>
              <div style={{ fontSize: "var(--fs-sm)", fontWeight: 600, color: "var(--text)" }}>{meActor.name}</div>
              <div style={{ fontSize: "var(--fs-micro)", color: "var(--text-faint)" }}>human · coordinator</div>
            </div>
          </div>
        </div>
      </aside>
      ...
      <main className="main">
        <header className="topbar">
          <h1>{view === "inbox" ? "Needs you" : view === "board" ? "Board" : "Task"}</h1>
          {view === "inbox" && <span className="sub">your attention queue</span>}
          {view === "board" && <span className="sub">all work across {projects.length} repos</span>}
          <div className="spacer" />
          <div className={`sync ${syncing ? "syncing" : ""}`} title="local store polling">
            <span className="dot" />
            <Icon name="sync" size={13} />
            {syncing ? "syncing…" : `synced ${syncSecs < 5 ? "just now" : syncSecs + "s ago"}`}
          </div>
          <button className="iconbtn bordered" title="Toggle theme" onClick={() => setDark((d) => !d)}>
      ...
      {modal && (
        <NoteModal
          ...
          onConfirm={(note) => applyTransition(modal.task, modal.transition, note)}
        />
      )}
