...
const PATHS: Record<string, string> = {
  ...
  branch: "M6 4v12M6 20a2 2 0 1 0 0-4 2 2 0 0 0 0 4ZM6 6a2 2 0 1 0 0-4 2 2 0 0 0 0 4ZM18 8a2 2 0 1 0 0-4 2 2 0 0 0 0 4ZM18 6c0 4-6 3-6 8",
  ...
  worktree: "M5 3v4a2 2 0 0 0 2 2h10a2 2 0 0 0 2-2V3M12 9v12M8 21h8M5 12l-2 2 2 2",
...
export function StateBadge({ state, size = "md", solid = false }: { state: State; size?: "sm" | "md"; solid?: boolean }) {
...
export function Avatar({
  ...
  const a = (actorId && actors[actorId]) || { short: "??", kind: "agent" as const, name: actorId || "?" };
...
export function ProjectTag({ project, onClick, active }: { project: string; onClick?: (e: React.MouseEvent) => void; active?: boolean }) {
  return (
    <button className={`proj-tag ${active ? "is-active" : ""}`} onClick={onClick} title={"git repo: " + project}>
...
export function MoveMenu({
  ...
  return (
    <span className="movemenu" ref={ref} onClick={(e) => e.stopPropagation()}>
      <button className={`movemenu-trigger ${open ? "is-open" : ""}`} onClick={() => setOpen((o) => !o)} title="Change state">
        <StateBadge state={task.state} size="sm" />
        <Icon name="chevDown" size={12} />
      </button>
      {open && (
        <div className={`movemenu-pop ${align === "right" ? "align-right" : ""}`}>
          ...
          {trans.map((t) => (
            ...
            >
              <Icon name={t.icon} size={13} style={{ color: "var(--text-faint)" }} />
              <StateBadge state={t.to} size="sm" />
              {t.requiresNote && <span className="movemenu-note">needs note</span>}
            </button>
          ))}
        </div>
      )}
