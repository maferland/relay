  ...
  switch (state) {
    case "todo":
      return [
        ...
        { to: "blocked", label: "Block", icon: "alert", requiresNote: true, danger: true },
      ];
    case "doing":
      return [
    ...
    case "review":
      return [
        { to: "done", label: "Pass QA", icon: "check", primary: true, good: true },
        { to: "todo", label: "Reject", icon: "arrowLeft", requiresNote: true, danger: true },
        { to: "blocked", label: "Block", icon: "alert", requiresNote: true },
      ];
    case "blocked":
      return [
        { to: "doing", label: "Unblock & resume", icon: "arrowRight", primary: true },
        { to: "todo", label: "Send to backlog", icon: "arrowLeft" },
      ];
    case "done":
      return [{ to: "doing", label: "Reopen", icon: "sync", requiresNote: true }];
    default:
      return [];
  }
}
...
export function transitionMeta(from: State, to: State): Transition {
  ...
  return {
    ...
    good: to === "done",
    label,
    icon: to === "blocked" ? "alert" : backward ? "arrowLeft" : to === "done" ? "check" : "arrowRight",
  };
}
