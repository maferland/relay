...
export function Board({ tasks, actors, me, projects, now, onOpen, onAction }: BoardProps) {
  ...
  const filtered = useMemo(() => {
    ...
    return tasks
      .filter((t) => {
        ...
        if (ql) {
          const hay = (t.title + " " + t.id + " " + t.project + " " + (t.assignee || "") + " " + (t.branch || "")).toLowerCase();
  ...
  useEffect(() => {
    const onKey = (e: KeyboardEvent) => {
      ...
      } else if ((e.key === "e" || e.key === "Enter") && sel >= 0 && filtered[sel]) {
        e.preventDefault();
    ...
    <div className="page page-wide">
      <div className="board-toolbar">
        <div className="search">
          ...
          <input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Search tasks, ids, branches…" />
          {q && (
            <span className="iconbtn" style={{ width: 20, height: 20 }} onClick={() => setQ("")}>
              <Icon name="x" size={13} />
            </span>
          )}
        </div>
        <div className="segmented">
          {BOARD_STATES.map((s) => (
            <button key={s} className={`seg ${states.has(s) ? "is-active" : ""}`} onClick={() => toggleState(s)}>
              <span className="badge-dot" style={{ background: `var(--st-${s}-fg)` }} />
              {STATE_META[s].label}
        ...
        <div className="segmented">
          <button className={`seg ${bview === "list" ? "is-active" : ""}`} onClick={() => setBview("list")}>
            <Icon name="list" size={13} /> List
          </button>
          <button className={`seg ${bview === "kanban" ? "is-active" : ""}`} onClick={() => setBview("kanban")}>
            <Icon name="board" size={13} /> Board
          </button>
        </div>
        {bview === "list" && (
          <button className={`chip ${grouped ? "is-active" : ""}`} onClick={() => setGrouped((g) => !g)}>
      ...
      <div className="filterbar">
        ...
        <button className={`chip ${!proj ? "is-active" : ""}`} onClick={() => setProj(null)}>
          all
        </button>
        {projects.map((p) => (
          <button key={p} className={`chip ${proj === p ? "is-active" : ""}`} onClick={() => setProj(proj === p ? null : p)}>
            <span className="mono">{p}</span>
          </button>
        ))}
        <span style={{ width: 8 }} />
        <button className={`chip ${mineOnly ? "is-active" : ""}`} onClick={() => setMineOnly((m) => !m)}>
          <Icon name="user" size={13} /> assigned to me
        </button>
        <span style={{ width: 8 }} />
        <span className="filter-label">updated</span>
        {SINCE_OPTS.map((s) => (
          <button key={s.k} className={`chip ${since === s.k ? "is-active" : ""}`} onClick={() => setSince(s.k)}>
            {s.label}
          </button>
        ))}
        <div className="spacer" />
      ...
      {filtered.length === 0 ? (
        <div className="empty">
          ...
          <p>
            Try clearing a filter or widening the time window. {tasks.length} tasks exist across {projects.length} repos.
          </p>
        </div>
      ) : bview === "kanban" ? (
        <Kanban tasks={filtered} actors={actors} me={me} onOpen={onOpen} onAction={onAction} />
      ) : (
          ...
          <div className="board-group" key={name || "all"}>
            ...
            <div className="rows">
              {rows.map((t) => {
                ...
                const needsMe = t.needsHuman || t.state === "blocked" || (t.state === "review" && t.assignee === me);
                return (
                  <div
                    key={t.id}
                  ...
                  >
                    ...
                    <div className="row-main">
                      ...
                      <div className="row-meta">
                        ...
                        {!grouped && (
                          <span className="meta">
                            <Icon name="folder" size={11} /> <span className="mono">{t.project}</span>
                          </span>
                        )}
                        {t.branch && <span className="dotsep" />}
                        {t.branch && (
                          <span className="meta row-branch">
                            <Icon name="branch" size={11} /> <span className="mono">{t.branch}</span>
                          </span>
                        )}
                      </div>
                    </div>
                    <div className="row-right">
                      {needsMe && <span className="row-att" title="needs you" />}
                      {t.assignee ? (
                        <Avatar actorId={t.assignee} actors={actors} size={20} />
                      ) : (
                        <span className="meta" style={{ fontSize: "var(--fs-xs)" }}>
                          unassigned
                        </span>
                      )}
                      <span className="row-time mono">{relTime(t.updatedAt, now)}</span>
                    </div>
                  </div>
                );
              })}
      ...
      {bview === "list" ? (
        <div style={{ display: "flex", gap: 14, marginTop: 18, color: "var(--text-faint)", fontSize: "var(--fs-xs)", alignItems: "center" }}>
          <span>Navigate</span>
          ...
          <span style={{ marginLeft: 8 }}>· click a state badge to move a task</span>
        </div>
      ) : (
        <div style={{ display: "flex", gap: 8, marginTop: 16, color: "var(--text-faint)", fontSize: "var(--fs-xs)", alignItems: "center" }}>
          <Icon name="list" size={13} /> Drag a card between columns to change its state. Moving backward or to Blocked asks for a note.
        </div>
      )}
    </div>
  );
