  ...
  return (
    ...
    >
      ...
      <div className="kcard-foot">
        ...
        {task.assignee ? (
          <Avatar actorId={task.assignee} actors={actors} size={20} />
        ) : (
          <span className="kdash" />
        )}
      </div>
    </div>
  )
...
export function Kanban({ tasks, actors, me, onOpen, onAction }: KanbanProps) {
  ...
  const handleDrop = (e: React.DragEvent, state: State) => {
    e.preventDefault()
    const id =
      dragId || (e.dataTransfer && e.dataTransfer.getData('text/plain'))
    setOverCol(null)
    setDragId(null)
    ...
    return (
      <div
        ...
        onDragLeave={(e) => {
          if (e.currentTarget === e.target)
            setOverCol(state === overCol ? null : overCol)
        }}
      ...
      >
        <div className="kcol-head">
          <span
            className="badge-dot"
            style={{ background: `var(--st-${state}-fg)` }}
          />
        ...
        <div className="kcol-body">
          ...
          {valid && back && (
            <div className="kdrop">
              ...
              {back.requiresNote
                ? 'Drop — needs a note'
                : 'Drop to ' + STATE_META[state].label.toLowerCase()}
            </div>
          )}
