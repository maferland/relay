...
describe("SqliteTaskStore", () => {
  ...
  it("records a state transition with from/to in history", async () => {
    ...
    const updated = await store.update("task-1", { state: "review" }, { actor: "worker", note: "ready" });
    expect(updated.state).toBe("review");
    expect(updated.history.at(-1)).toMatchObject({ from: "todo", to: "review", actor: "worker", note: "ready" });
  });

  it("bumps updatedAt and records a note-only event when state is unchanged", async () => {
    await store.add(makeTask("task-1", { updatedAt: "2020-01-01T00:00:00.000Z" }));
    const updated = await store.update("task-1", { assignee: "worker-2" }, { actor: "lead", note: "reassigned" });
  ...
  it("requires a note to send a task back from review", async () => {
    ...
    await expect(store.update("task-1", { state: "todo" }, { actor: "qa" })).rejects.toThrow(/note is required/);
    const withNote = await store.update("task-1", { state: "todo" }, { actor: "qa", note: "missing tests" });
    expect(withNote.history.at(-1)).toMatchObject({ from: "review", to: "todo", note: "missing tests" });
  });

  it("requires a note to block a task but not to move forward", async () => {
    await store.add(makeTask("task-1"));
    await expect(store.update("task-1", { state: "blocked" }, { actor: "w" })).rejects.toThrow(/note is required/);
    expect((await store.update("task-1", { state: "doing" }, { actor: "w" })).state).toBe("doing");
  });

  it("throws updating a missing task", async () => {
    await expect(store.update("task-x", { state: "done" }, {})).rejects.toThrow(/not found/);
  });
  ...
  it("refuses to add a task whose id already exists (never clobbers history)", async () => {
    ...
    await expect(store.add(makeTask("task-1", { title: "other" }))).rejects.toThrow(/already exists/);
  ...
  it("rejects an invalid since timestamp", async () => {
    await expect(store.list({ since: "not-a-date" })).rejects.toThrow(/Invalid since/);
  });
  ...
  it("clears assignee/description when given an empty string", async () => {
    ...
    const cleared = await store.update("task-1", { assignee: "", description: "" }, { actor: "lead" });
  ...
  describe("claim", () => {
    it("assigns and moves an open task to doing", async () => {
      await store.add(makeTask("task-1"));
      const claimed = await store.claim("task-1", { assignee: "w1", actor: "w1" });
    ...
    it("refuses to claim a task in review or done", async () => {
      ...
      await expect(store.claim("task-r", { assignee: "w1" })).rejects.toThrow(/reopen/);
      await expect(store.claim("task-d", { assignee: "w1" })).rejects.toThrow(/reopen/);
    });
  });
  ...
  describe("buildTask", () => {
    it("requires a note to create a task already blocked", () => {
      ...
      expect(buildTask({ title: "x", project: "p", state: "blocked", note: "waiting on API" }).state).toBe("blocked");
    });

    it("rejects an empty title and normalizes empty optionals to undefined", () => {
      expect(() => buildTask({ title: "  ", project: "p" })).toThrow(/title is required/);
      const t = buildTask({ title: "x", project: "p", assignee: "", description: "" });
  ...
  describe("list filtering", () => {
    ...
    beforeEach(async () => {
      await store.add(makeTask("task-a", { state: "todo", project: "alpha", assignee: "w1", updatedAt: past }));
      await store.add(makeTask("task-b", { state: "review", project: "alpha", assignee: "w2", updatedAt: past }));
      await store.add(makeTask("task-c", { state: "review", project: "beta", assignee: "w1", updatedAt: past }));
    });

    it("filters by state", async () => {
      expect((await store.list({ state: "review" })).map((t) => t.id).sort()).toEqual(["task-b", "task-c"]);
    });

    it("filters by project and assignee together", async () => {
      expect((await store.list({ project: "alpha", assignee: "w2" })).map((t) => t.id)).toEqual(["task-b"]);
    });
  ...
  describe("concurrency", () => {
    it("does not drop history under concurrent updates to the same task", async () => {
      ...
      await Promise.all(
        Array.from({ length: 30 }, (_, i) => store.update("task-hot", {}, { actor: `w${i}`, note: `n${i}` }))
      );
