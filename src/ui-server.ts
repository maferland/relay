...
function actorsFrom(tasks: Task[], me: string) {
  ...
  const actors: Record<string, { name: string; kind: "human" | "agent"; short: string }> = {};
  for (const name of names) {
    actors[name] = { name, kind: name === me ? "human" : "agent", short: initials(name) };
  }
  return actors;
}
...
export function createUiServer(store: SqliteTaskStore, opts: UiServerOptions) {
  ...
  const server = Bun.serve({
    ...
    async fetch(req) {
      ...
      const match = p.match(/^\/api\/tasks\/([^/]+)\/(transition|escalate|resolve)$/);
      if (match && req.method === "POST") {
        ...
        try {
          if (action === "transition") {
            if (!body.to || !isState(body.to)) return json({ error: "Invalid target state" }, 400);
            const to = body.to as State;
            const current = await store.get(id);
            const changes: { state: State; assignee?: string } = { state: to };
            if (to === "doing" && current && !current.assignee) changes.assignee = me;
            const updated = await store.update(id, changes, { actor: me, note: body.note });
            return json(adapt(updated));
          }
          if (action === "escalate") {
            return json(adapt(await store.escalate(id, { actor: me, note: body.note })));
          }
          return json(adapt(await store.resolve(id, { actor: me, note: body.note })));
        } catch (e) {
