...
describe("tasks CLI", () => {
  ...
  async function addTask(title: string, extra: string[] = []): Promise<Task> {
    const { stdout, exitCode } = await run(["add", title, "--project", "demo", "--json", ...extra]);
  ...
  it("runs the full QA-handoff loop", async () => {
    ...
    const claimed = JSON.parse((await run(["claim", id, "--json"], "worker-1")).stdout) as Task;
    expect(claimed.state).toBe("doing");
    expect(claimed.assignee).toBe("worker-1");

    const review = JSON.parse(
      (await run(["update", id, "--state", "review", "--note", "ready for QA", "--json"], "worker-1")).stdout
    ) as Task;
    expect(review.state).toBe("review");

    // coordinator polls for the handoff
    const list = JSON.parse(
      (await run(["list", "--state", "review", "--project", "demo", "--json"], "lead")).stdout
    ) as Task[];
    expect(list.map((t) => t.id)).toContain(id);

    const done = JSON.parse(
      (await run(["update", id, "--state", "done", "--note", "QA passed", "--json"], "lead")).stdout
    ) as Task;
    expect(done.state).toBe("done");
    expect(done.history.map((e) => e.to)).toEqual(["todo", "doing", "review", "done"]);
  });

  it("requires a note when QA sends a task back, and records the exchange", async () => {
    const { id } = await addTask("fix login", ["--assignee", "worker-1"]);
    await run(["update", id, "--state", "review", "--note", "ready", "--json"], "worker-1");
    ...
    const rejected = JSON.parse(
      (await run(["update", id, "--state", "todo", "--note", "missing tests", "--json"], "qa")).stdout
    ) as Task;
    expect(rejected.state).toBe("todo");
    expect(rejected.history.at(-1)).toMatchObject({ from: "review", to: "todo", actor: "qa", note: "missing tests" });
  });

  it("records plan and clears assignee with an empty string", async () => {
    const created = await addTask("ship it", ["--plan", "1. write 2. test", "--assignee", "bob"]);
  ...
  it("requires a note to create a task directly in blocked", async () => {
    const { exitCode, stderr } = await run(["add", "stuck", "--project", "demo", "--state", "blocked"]);
  ...
  describe("arg parsing", () => {
    ...
    it("errors when a value flag is missing its value", async () => {
      expect((await run(["add", "x", "--project", "demo", "--note"])).exitCode).toBe(2);
      expect((await run(["add", "x", "--project", "demo", "--assignee", "--json"])).exitCode).toBe(2);
    });

    it("rejects an empty --state", async () => {
      const { exitCode, stderr } = await run(["add", "x", "--project", "demo", "--state", ""]);
      expect(exitCode).toBe(2);
      expect(stderr).toMatch(/Invalid --state/);
    });

    it("treats tokens after -- as the title", async () => {
      const { stdout, exitCode } = await run(["add", "--project", "demo", "--json", "--", "--urgent", "fix", "it"]);
      expect(exitCode).toBe(0);
  ...
  it("errors on a missing task id", async () => {
    const { exitCode, stderr } = await run(["update", "task-missing", "--state", "done"]);
    expect(exitCode).toBe(1);
  ...
  it("keeps every history entry under concurrent updates from separate processes", async () => {
    ...
    await Promise.all(
      Array.from({ length: 20 }, (_, i) => run(["update", id, "--note", `n${i}`], `w${i}`))
    );
